...
def ann(model, label):
    ...
    if model == 'FFF':
        ...
        x = tf.keras.layers.Dense(64 * label, activation = tf.keras.activations.tanh, kernel_regularizer = tf.keras.regularizers.l2(REGULARIZATION_RATE), bias_regularizer = tf.keras.regularizers.l2(REGULARIZATION_RATE))(input)
        x = tf.keras.layers.Dense(64 * label, activation = tf.keras.activations.tanh, kernel_regularizer = tf.keras.regularizers.l2(REGULARIZATION_RATE), bias_regularizer = tf.keras.regularizers.l2(REGULARIZATION_RATE))(x)
        output = tf.keras.layers.Dense(1, activation = tf.keras.activations.relu, kernel_regularizer = tf.keras.regularizers.l2(REGULARIZATION_RATE), bias_regularizer = tf.keras.regularizers.l2(REGULARIZATION_RATE))(x)
        ...
        x = tf.keras.layers.Conv1D(2 * label, 3, activation = tf.keras.activations.tanh, kernel_regularizer = tf.keras.regularizers.l2(REGULARIZATION_RATE), bias_regularizer = tf.keras.regularizers.l2(REGULARIZATION_RATE))(x)
        x = tf.keras.layers.Conv1D(2 * label, 3, activation = tf.keras.activations.tanh, kernel_regularizer = tf.keras.regularizers.l2(REGULARIZATION_RATE), bias_regularizer = tf.keras.regularizers.l2(REGULARIZATION_RATE))(x)
        ...
        input = tf.keras.Input(shape = (LENGTH_OF_TIME,), dtype = tf.float32)
        x = tf.keras.layers.Reshape((LENGTH_OF_TIME, 1), input_shape=(LENGTH_OF_TIME,))(input)
        x = tf.keras.layers.LSTM(64 * label, return_sequences = True, kernel_regularizer = tf.keras.regularizers.l2(REGULARIZATION_RATE), recurrent_regularizer = tf.keras.regularizers.l2(REGULARIZATION_RATE), bias_regularizer = tf.keras.regularizers.l2(REGULARIZATION_RATE))(x)
        x = tf.keras.layers.LSTM(64 * label, kernel_regularizer = tf.keras.regularizers.l2(REGULARIZATION_RATE), recurrent_regularizer = tf.keras.regularizers.l2(REGULARIZATION_RATE), bias_regularizer = tf.keras.regularizers.l2(REGULARIZATION_RATE))(x)
        output = tf.keras.layers.Dense(1, activation=tf.keras.activations.relu, kernel_regularizer = tf.keras.regularizers.l2(REGULARIZATION_RATE), bias_regularizer = tf.keras.regularizers.l2(REGULARIZATION_RATE))(x)


    model = tf.keras.Model(input, output)
#     model.compile(optimizer = tf.keras.optimizers.SGD(learning_rate=LEARNING_RATE, momentum=0.9, nesterov=True), loss = tf.keras.losses.MAE)
...
hist = np.column_stack((history.history['loss'], history.history['val_loss']))[:-MAX_EPOCH]
