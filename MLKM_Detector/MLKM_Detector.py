...
TRIAL = int(sys.argv[14])
TRAIN_LABEL_INI = int(sys.argv[15])
TRAIN_LABEL_END = int(sys.argv[16])
...
dir_a = '/pds/pds151/ckj/MLKM/MLKM_Detector_6/saves/'
dir_b = '/pds/pds151/ckj/MLKM/MLKM_Detector_6/saves/N{N:d}K{K:0.4f}L{L:d}/'
dir_c = '/pds/pds151/ckj/MLKM/MLKM_Detector_6/saves/N{N:d}K{K:0.4f}L{L:d}/{model:s}_{label:d}/'

if not os.path.exists(dir_a):
    os.mkdir(dir_a)
if not os.path.exists(dir_b.format(N = SYSTEM_SIZE, K = COUPLING, L = LENGTH_IN)):
    os.mkdir(dir_b.format(N = SYSTEM_SIZE, K = COUPLING, L = LENGTH_IN))
if not os.path.exists(dir_c.format(N = SYSTEM_SIZE, K = COUPLING, L = LENGTH_IN, model = MODEL, label = LABEL)):
    os.mkdir(dir_c.format(N = SYSTEM_SIZE, K = COUPLING, L = LENGTH_IN, model = MODEL, label = LABEL))
...
struct = '/pds/pds151/ckj/MLKM/MLKM_Detector_6/saves/N{N:d}K{K:0.4f}L{L:d}/{model:s}_{label:d}/struct.png'
tf.keras.utils.plot_model(model, to_file = struct.format(N = SYSTEM_SIZE, K = COUPLING, L = LENGTH_IN, model = MODEL, label = LABEL), show_shapes = True, show_layer_names = False, rankdir = 'LR')
